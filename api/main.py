            ...
            if request.amazon_names:
                if len(request.amazon_urls) != len(request.amazon_names):
                    return JSONResponse(
                        ...
                        content={"detail": "Number of Amazon names must match number of URLs"}
                    )
        ...
        if snapshot_id:
            ...
            if min_profit_percent is not None or min_profit_amount is not None:
                opportunities = [
                    ...
                    if (min_profit_percent is None or opp.profit_percent >= min_profit_percent)
                    and (min_profit_amount is None or opp.profit_amount >= min_profit_amount)
                ]
...
async def get_snapshots(
    ...
    try:
        snapshots = db.query(Snapshot).order_by(Snapshot.timestamp.desc()).limit(limit).all()
        
        # Count items for each snapshot
        result = []
        for snapshot in snapshots:
            item_count = db.query(DBItem).filter(DBItem.snapshot_id == snapshot.id).count()
...
async def detect_opportunities(
    ...
    try:
        ...
        if request.snapshot_id or request.use_latest:
            ...
            db_items = db.query(DBItem).filter(DBItem.snapshot_id == active_snapshot_id).all()
        ...
        # Find opportunities
        detector = ArbitrageDetector(min_profit_percent=request.min_profit_percent or 5.0)
        ...
        if active_snapshot_id and opportunities:
            ...
            return OpportunityResponse(
                opportunities=[Opportunity.model_validate(opp) for opp in db_opportunities],
